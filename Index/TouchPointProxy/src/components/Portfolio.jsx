...
export default function ServicesPortfolio(props) {
    ...
    React.useEffect(() => {
        ...
        const url = `${PROXY_HOSTNAME}/service/portfolio`
        const portfolio = async () => fetch(url)
        portfolio()
            .then(res => res.json())
            .then(data => {
...
function ServicePortfolioForm (props){
    ...
    return (
        <Box>
            <Card>
                <CardContent>
                <form action={`${PROXY_HOSTNAME}/service/portfolio/update`} method="post" encType="multipart/form-data">
